...
# ADD BOILERPLATE HERE (__eq__ and __repr__ functions)
    def __eq__(self,other):
        if not isinstance(other,Location):
            ...
            return self.name.lower() == other.name.lower() and math.isclose(self.lat,other.lat) and math.isclose(self.lon,other.lon)

    def __repr__(self):
        return 'Location({0:s}, {1:.1f}, {2:.1f})'.format(self.name,self.lat,self.lon)
def main():
